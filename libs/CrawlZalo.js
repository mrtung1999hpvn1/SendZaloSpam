...
`
            ...
            if (result.status() === 404) {
                ...
                // 034 5 990 398
                var mySavePoint = fs.readFileSync('./libs/SavePoint.txt');
                
                for (let i = parseInt(mySavePoint) ; i <= 6000000; i++) {
                    try{
                        const phoneNumber = `034` + "0".repeat(7 - (i).toString().length) + i
                        fs.writeFile('./libs/SavePoint.txt', i.toString(), function (err, result) {
                        ...
                        if (parseInt(numberOfDivs) > 0) {
                            ...
                            const numberOfStt = await page.evaluate(_ => {
                                return document.querySelectorAll('.stt .v3').length;
                            });
                            var sttProfile = ' '
                            console.log('stt number : '+ numberOfStt)
    
                            sttProfile = await page.evaluate(_ => {
                                return document.querySelector('#profilePhoto').innerText;
                            })
                            ...
                            if(checkQuery.rows.length > 0){
                                const update = await pool.query(`
                                    update chatzalo set name=N'${crawed.split('\n')[0]}',image=N'${backgroundImageUrl}',profile=N'${crawledProfile.profile}',
                                    phone=N'${phoneNumber}',note= N'${sttProfile.toString()}',sex=N'${crawledProfile.profile.indexOf('Nữ') > 0 ? "Nữ" : 'Nam'}',birthday=N'${crawledProfile.profile.split('Ngày sinh\n')[1]}'
                                ...
                                values(
                                    ...
                                    N'${crawledProfile.profile}',
                                    N'${phoneNumber}',
                                    N'${sttProfile}',
                                    N'${crawledProfile.profile.indexOf('Nữ') > 0 ? "Nữ" : 'Nam'}',
                                    N'${crawledProfile.profile.split('Ngày sinh\n')[1]
                                }')
                            `)
                                ...
                                //     profile: crawledProfile.profile,
                                //     phone: phoneNumber,
                                //     note: sttProfile,
                                //     sex: crawledProfile.profile.indexOf('Nữ') > 0 ? "Nữ" : 'Nam',
                                //     birthday: crawledProfile.profile.split('Ngày sinh\n')[1]
                                // };
                                // console.log(body)
                                // fetch('http://103.127.207.24:3011/0366262072/ChatZalo', {
                                // 		method: 'post',
    ...
    async sendZalo() {
        try {
            ...
            if (result.status() === 404) {
                ...
                var mySavePoint = fs.readFileSync('./libs/SavePointSend.txt');
                
                for (let i = parseInt(mySavePoint) ; i <= newData.length ; i++){

                    fs.writeFile('./libs/SavePointSend.txt', i.toString(), function (err, result) {
                        if (err) console.log('error', err);
                    });
                    ...
                    // Tin nhắn gửi đi ứng với mỗi dùng là một lần tin nhắn gửi đi
                    var Message =
                    `
                    Xin chào bạn ${newData[i].name} nhé ! Cảm ơn bạn đã xem tin nhắn này. \n
                    Hiện tại mình đang làm về mảng dịch vụ studio và giải trí tại Hải Phòng \n
                    Bạn quan tâm có thể nhấn theo dõi và ủng hộ team mình nhé \n
                    https://www.facebook.com/99plusstudioo \n
                    Thân
                    `
                    ...
                    console.log(parseInt(numberOfDivs))
                    if (parseInt(numberOfDivs) > 0) {
                        
                        await page.click('.list-friend-conctact')

                        sleep(1500)

                        await page.click('#input_line_0')

                        await page.keyboard.type(Message , {delay: 1})
                        
                        sleep(3000)

                        await page.click('.z--btn--text--tertiary')
                    } else {
                    }
                }
            }
